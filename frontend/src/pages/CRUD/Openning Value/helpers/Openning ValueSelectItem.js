...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/Openning Value/autocomplete`, { params });
  return response.data;
}

class Openning ValueSelectItem extends Component {
  ...
  mapper = {
    intoSelect(originalValue) {
      ...
      let label = originalValue.label ? originalValue.label : originalValue.Concatenate;
...
const select = (state) => ({
  hasPermissionToCreate: state.Openning Value.hasPermissionToCreate
});
