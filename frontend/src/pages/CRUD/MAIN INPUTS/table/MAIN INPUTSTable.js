...
import * as dataFormat from 'pages/CRUD/MAIN INPUTS/table/MAIN INPUTSDataFormatters';


import actions from 'actions/MAIN INPUTS/MAIN INPUTSListActions';
...
class MAIN INPUTSTable extends Component {
  constructor(props) {
  super(props);
    this.state = {
    modalOpen: false,
    idToDelete: null,
    filters: [
      {label: 'Enquiry No', title: 'Enquiry No'},{label: 'Project Name', title: 'Project Name'},{label: 'CODE', title: 'CODE', number: 'true'},
    ],
    filterItems: [],
  }
...
{ null && <Button
          color="default"
          size="xs"
          onClick={() => this.props.dispatch(push(`/admin/MAIN INPUTS/${cell}`))}
        >
      View
      </Button>
}
        <Button
          color="info"
          size="xs"
          onClick={() => this.props.dispatch(push(`/admin/MAIN INPUTS/${cell}/edit`))}
        >
        Edit
      </Button>
      &nbsp;&nbsp;
  ...
  render() {
    ...
    const options = {
      ...
      onRowClick: function(row){
        return dispatch(push(`/admin/MAIN INPUTS/${row.id}/edit`))
      },
    };

    return (
      <div>
        <Widget title={"MAIN INPUTS"} collapse close>
          <Link to="/admin/MAIN INPUTS/new">
            <button
          ...
          <BootstrapTable bordered={false} data={rows} version="4" pagination options={options} trStyle={this.rowStyleFormat} search tableContainerClass={`table-responsive table-striped table-hover`}>
              ...
              <TableHeaderColumn dataField="Enquiry No" dataSort
                
              >
                <span className="fs-sm">Enquiry No</span>
              </TableHeaderColumn>
            
              <TableHeaderColumn dataField="Project Name" dataSort
                
              >
                <span className="fs-sm">Project Name</span>
              </TableHeaderColumn>
...
function mapStateToProps(store) {
  return {
    loading: store.MAIN INPUTS.list.loading,
    rows: store.MAIN INPUTS.list.rows,
    modalOpen: store.MAIN INPUTS.list.modalOpen,
    idToDelete: store.MAIN INPUTS.list.idToDelete,
  };
}

export default connect(mapStateToProps)(MAIN INPUTSTable);
